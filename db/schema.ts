...
export const projectType = ["feature", "tv_pilot", "tv_season", "short", "stage_play", "other"] as const;
export const scriptFormat = ["screenplay", "stage_play", "other"] as const;
...
export const campaigns = pgTable(
  "campaigns",
  {
    ...
    name: text("name").notNull(),
    /** Jack Script: project type (feature, pilot, etc.) */
    type: text("type", { enum: projectType }).default("feature"),
    logline: text("logline"),
    genre: text("genre"),
    targetPageCount: integer("target_page_count"),
    format: text("format", { enum: scriptFormat }).default("screenplay"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => ({
...
export const references = pgTable(
  "references",
  {
    ...
    // doc = Project Bible (flexible JSONB):
    // logline, synopsis, characters[], locations[], toneBible, visualLanguage,
    // beatSheet, themes[], worldRules, redLines, gateSpec (+ legacy editorial keys)
...
export const pieces = pgTable(
  "pieces",
  {
    ...
    format: text("format", { enum: scriptFormat }).default("screenplay"),
    pageEstimate: real("page_estimate"),
    sceneCount: integer("scene_count"),
    parsedScenes: jsonb("parsed_scenes"),
