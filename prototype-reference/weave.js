/* ============================================================
   ...
   Pipeline (every step bounded so file count/length never truncates):
     ...
     EXPAND draftSection()       one call per structure section -> unified draft
   ============================================================ */
(function () {
...
{"summary":"1-2 sentences","themes":["3-5 short theme phrases"],"claims":["2-4 key claims, short"],"signals":["1-3 angle/throughline signals — what bigger idea this points to"],"lines":["1-2 notable verbatim lines worth keeping"]}`;
    ...
    const r = await window.AI.json(prompt, { system });
    return {
      name: source.name,
      summary: r.summary || "",
      themes: r.themes || [],
  ...
  function extractsBlock(extracts) {
...
`[S${i + 1}] ${e.name}
  ...
  async function synthesizeBrief(extracts) {
    const system =
...
Make 'structure' a 3-6 part outline for a single essay.`;
    ...
    const r = await window.AI.json(prompt, { system });
    r.workingTitle = r.workingTitle || "Untitled weave";
    r.structure = Array.isArray(r.structure) && r.structure.length ? r.structure : [{ section: "Whole", purpose: "Make the case", draws: [] }];
    r.tensions = r.tensions || [];
    return r;
  }

  async function mapToThroughlines(brief, refCtx) {
    const system =
...
{"mapped":[{"tag":"<throughline tag>","how":"<how the concept serves it, 1 sentence>"}],"nearestAngle":"<only if fit is weak, else null>","audience":"<best-fit audience name>","register":"<essay or field>"}`;
  ...
  function digestFor(draws, extracts) {
    const idxs = (draws || []).map((d) => parseInt(String(d).replace(/[^0-9]/g, ""), 10) - 1).filter((n) => n >= 0 && n < extracts.length);
    const chosen = idxs.length ? idxs.map((i) => extracts[i]) : extracts;
    return chosen.map((e) =>
`• ${e.name}: ${e.summary} ${(e.claims || []).join("; ")} ${(e.lines || []).map((l) => `“${l}”`).join(" ")}`).join("\n");
  }

  async function draftSection(section, idx, total, brief, mapping, extracts, refCtx) {
...
Write the section now in the delimited format.`;
  ...
  async function runWeave(sources, refCtx, onProgress) {
    ...
    for (let i = 0; i < usable.length; i++) {
      ...
      try { extracts.push(await extractSource(usable[i])); }
    ...
    const brief = await synthesizeBrief(extracts);

    // MAP2 — map onto references
    if (onProgress) onProgress({ phase: "map" });
    let mapping = { mapped: [], nearestAngle: null, audience: "", register: "essay" };
    try { mapping = await mapToThroughlines(brief, refCtx); } catch (e) { console.warn("map failed", e); }
    ...
    for (let i = 0; i < brief.structure.length; i++) {
      ...
      try { sections.push(await draftSection(brief.structure[i], i, brief.structure.length, brief, mapping, extracts, refCtx)); }
  ...
  window.WEAVE = { runWeave, extractSource, synthesizeBrief, mapToThroughlines, draftSection };
})();
