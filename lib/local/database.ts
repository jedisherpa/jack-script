...
import { CAMPAIGN_NAMES, SEED_REFERENCES, slug } from "@/lib/seed-data";
...
function migrateLooseColumns(db: Database.Database): void {
  ...
  addColumn(db, "campaigns", "type", "TEXT DEFAULT 'feature'");
  addColumn(db, "campaigns", "logline", "TEXT");
  addColumn(db, "campaigns", "genre", "TEXT");
  addColumn(db, "campaigns", "target_page_count", "INTEGER");
  addColumn(db, "campaigns", "format", "TEXT DEFAULT 'screenplay'");
  addColumn(db, "pieces", "format", "TEXT DEFAULT 'screenplay'");
...
function rowCampaign(row: any): LocalCampaign {
  return {
    ...
    targetPageCount: row.target_page_count == null ? null : Number(row.target_page_count),
    format: row.format ?? "screenplay",
...
function rowPiece(row: any): LocalPiece {
  return {
    ...
    status: row.status,
    format: row.format ?? "screenplay",
    pageEstimate: row.page_estimate == null ? null : Number(row.page_estimate),
    sceneCount: row.scene_count == null ? null : Number(row.scene_count),
    parsedScenes: parseJson(row.parsed_scenes_json, null),
...
export function ensureLocalWorkspace(userId = LOCAL_USER_ID): string {
  ...
  const tx = db.transaction(() => {
    ...
    const campaignCount = db.prepare("SELECT COUNT(*) AS count FROM campaigns WHERE workspace_id = ?")
      .get(LOCAL_WORKSPACE_ID) as { count: number };
    if (campaignCount.count === 0) {
      const insertCampaign = db.prepare(
        "INSERT INTO campaigns (id, workspace_id, slug, name) VALUES (?, ?, ?, ?)",
      );
      const insertReferences = db.prepare(
        "INSERT INTO references_doc (id, campaign_id, doc_json) VALUES (?, ?, ?)",
      );
      for (const name of CAMPAIGN_NAMES) {
        const campaignId = id("camp");
        insertCampaign.run(campaignId, LOCAL_WORKSPACE_ID, slug(name), name);
        insertReferences.run(id("ref"), campaignId, JSON.stringify(SEED_REFERENCES));
      }
    }
  });
  tx();
...
export function createLocalCampaign(input: { id?: string; name: string; workspaceId?: string }): LocalCampaign {
  ...
  const tx = db.transaction(() => {
    ...
    ).run(randomUUID(), campaignId, JSON.stringify(SEED_REFERENCES));
  });
  tx();
