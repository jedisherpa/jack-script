 ...
 */
import { z } from "zod";
import { pieceStatus, scriptFormat } from "@/db/schema";

/** POST /api/campaigns/:cid/pieces — create a script (status defaults to Draft). */

export const createPieceSchema = z.object({
  id: z.string().uuid().optional(),
  title: z.string().trim().min(1, "Title is required.").max(300),
  original: z.string().max(200_000).optional(),
  format: z.enum(scriptFormat).optional(),
});
export type CreatePieceInput = z.infer<typeof createPieceSchema>;

export const GATE_IDS = ["format", "character", "dialogue", "pacing", "visual", "theme", "market"] as const;
