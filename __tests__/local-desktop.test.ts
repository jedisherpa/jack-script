...
describe("local desktop database", () => {
  it("creates the default workspace and seed campaigns in SQLite", async () => {
    const { ensureLocalWorkspace, listLocalCampaigns } = await import("@/lib/local/database");

    expect(ensureLocalWorkspace()).toBe("local-workspace");
    const campaigns = listLocalCampaigns();

    expect(campaigns).toHaveLength(7);
    expect(campaigns[0]).toMatchObject({ name: "Untitled Feature", slug: "untitled-feature" });
  });
  ...
  it("supports the core local editorial workflow", async () => {
    ...
    expect(refs?.doc).toHaveProperty("logline");
