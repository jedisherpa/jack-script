# AGENTS.md — Jack Script build instructions

You are building **Jack Script**, a professional local-first screenwriting workstation repurposed from Pillar Press. Read `JACK_SCRIPT_MIGRATION.md`, `docs/DATA_MODEL.md`, and `docs/API_SPEC.md` before writing code.
...
| Area | Path |
|---|---|
| Coverage gates | `lib/gates.ts` |
| Revision | `lib/revision.ts` |
| Artifacts | `lib/generators.ts` |
| Bible context | `lib/refContext.ts` |
| Screenplay parse/export | `lib/screenplay/` |
| Seed Bible | `lib/seed-data.ts` |
| Local DB | `lib/local/database.ts` |

## Build order

1. Schema + migrations for project/script fields
2. Gate prompts + review route (incremental packet)
